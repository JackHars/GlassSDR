...
export function FlexTxApp() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>FLEX Transmitter</h2>
      <p style={{ color: "#aaa", fontSize: 13 }}>FLEX pager protocol — 1600 bps 2-FSK</p>

      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        <label>Capcode</label>
        <input type="number" value={capcode} onChange={(e) => setCapcode(e.target.value)} placeholder="0–2097151" style={inputStyle} />

        <label>Message</label>
        <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3} style={inputStyle} placeholder="Pager message..." />

        <label>Frequency (Hz)</label>
        <input type="number" value={frequency} onChange={(e) => setFrequency(e.target.value)} placeholder="e.g. 931962500" style={inputStyle} />
