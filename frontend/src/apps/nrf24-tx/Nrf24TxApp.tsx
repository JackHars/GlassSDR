...
const WARNING_BANNER = (
  <div style={{ background: "#220", border: "1px solid #fa0", borderRadius: 4, padding: "8px 12px", marginBottom: 12, color: "#fc8", fontSize: 13 }}>
    <strong>OWN DEVICES ONLY</strong> — nRF24 transmissions must only target devices you own and are licensed to operate.
  </div>
);
...
export function Nrf24TxApp() {
  ...
  const handleTransmit = async () => {
    if (!armed) return;
    await startApp("nrf24_tx" as any, {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>NRF24 TX</h2>
      {WARNING_BANNER}
      <p style={{ color: "#aaa", fontSize: 13 }}>nRF24L01+ Enhanced ShockBurst transmitter — 2.4 GHz ISM band</p>

      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        <label>Frequency (Hz)</label>
        <input type="number" value={frequency} onChange={(e) => setFrequency(e.target.value)} placeholder="e.g. 2400000000" style={inputStyle} />
