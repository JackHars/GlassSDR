...
export function Mdc1200TxApp() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>MDC-1200 TX</h2>
      {WARNING_BANNER}
      <p style={{ color: "#aaa", fontSize: 13 }}>MDC-1200 radio signaling — AFSK 1200/1800 Hz</p>

      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        <label>Unit ID (hex)</label>
        <input type="text" value={unitId} onChange={(e) => setUnitId(e.target.value)} placeholder="e.g. 0x1234" style={inputStyle} />

        <label>Opcode (hex)</label>
        <input type="text" value={opcode} onChange={(e) => setOpcode(e.target.value)} placeholder="e.g. 0x01" style={inputStyle} />

        <label>Frequency (Hz)</label>
        <input type="number" value={frequency} onChange={(e) => setFrequency(e.target.value)} placeholder="e.g. 154280000" style={inputStyle} />
