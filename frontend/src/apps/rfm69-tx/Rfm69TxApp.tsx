...
const WARNING_BANNER = (
  <div style={{ background: "#220", border: "1px solid #fa0", borderRadius: 4, padding: "8px 12px", marginBottom: 12, color: "#fc8", fontSize: 13 }}>
    <strong>OWN DEVICES ONLY</strong> — RFM69 transmissions must only target devices you own and are licensed to operate.
  </div>
);
...
export function Rfm69TxApp() {
  ...
  const handleTransmit = async () => {
    if (!armed) return;
    await startApp("rfm69_tx" as any, {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>RFM69 TX</h2>
      {WARNING_BANNER}
      <p style={{ color: "#aaa", fontSize: 13 }}>RFM69 FSK packet transmitter — 433/868/915 MHz ISM band</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        ...
        <label>Node Addr (hex)</label>
        <input type="text" value={nodeAddr} onChange={(e) => setNodeAddr(e.target.value)} placeholder="e.g. 0x01" style={inputStyle} />
