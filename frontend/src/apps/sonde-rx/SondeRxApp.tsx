...
export function SondeRxApp() {
  ...
  const handleStart = async () => {
    await startApp("sonde_rx" as AppId, { center_hz: freq, lna_gain_db: 32, vga_gain_db: 20, amp_enabled: false });
    setRunning(true);
  };
  ...
  return (
    <AppShell
      ...
      footer={<RecordBar appId={"sonde_rx" as any} format="jsonl" centerHz={freq} />}
    >
      <DecoderTable
        headers={["Serial", "Type", "Lat", "Lon", "Alt (m)"]}
        rows={frames}
        renderRow={(f) => [f.serial, f.sonde_type, f.lat.toFixed(5), f.lon.toFixed(5), f.alt_m.toFixed(1)]}
        emptyMessage="No telemetry yet — auto-detects RS41 / RS92 / DFM types."
      />
    </AppShell>
  );
}
