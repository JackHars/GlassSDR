...
interface P25Row extends DigitalVoiceEvent {
  nac: number;
  duid: number;
}
...
export function P25RxApp() {
  ...
  const handleStart = () =>
    startApp("p25_rx" as AppId, { center_hz: parseFloat(freq), lna_gain_db: 32, vga_gain_db: 20, amp_enabled: false });
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "#1c1c2c", textAlign: "left" }}>
              <th style={{ padding: "6px 8px" }}>NAC</th>
              <th style={{ padding: "6px 8px" }}>Talkgroup</th>
              <th style={{ padding: "6px 8px" }}>Source</th>
              <th style={{ padding: "6px 8px" }}>DUID</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #222" }}>
                <td style={{ padding: "4px 8px", fontFamily: "monospace" }}>0x{r.nac.toString(16).toUpperCase().padStart(3, "0")}</td>
                <td style={{ padding: "4px 8px", fontFamily: "monospace" }}>{r.talkgroup}</td>
                <td style={{ padding: "4px 8px", fontFamily: "monospace" }}>{r.source_id}</td>
                <td style={{ padding: "4px 8px", color: "#aaa" }}>{r.duid}</td>
              </tr>
            ))}
          </tbody>
        </table>
