...
interface NxdnRow extends DigitalVoiceEvent {
  ran: number;
}
...
export function NxdnRxApp() {
  ...
  const handleStart = () =>
    startApp("nxdn_rx" as AppId, { center_hz: parseFloat(freq), lna_gain_db: 32, vga_gain_db: 20, amp_enabled: false });
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #222" }}>
                <td style={{ padding: "4px 8px" }}>{r.ran}</td>
