...
interface NxdnRow extends DigitalVoiceEvent { ran: number; }

export function NxdnRxApp() {
  const [freq, setFreq] = useState("460000000");
  const [rows, setRows] = useState<NxdnRow[]>([]);
  const [running, setRunning] = useState(false);

  const handleStart = async () => {
    await startApp("nxdn_rx" as AppId, { center_hz: parseFloat(freq), lna_gain_db: 32, vga_gain_db: 20, amp_enabled: false });
    setRunning(true);
  };
  ...
  useEffect(() => {
    const unlisten = listen<DigitalVoiceEvent>("digital_voice", (e) => {
      if (e.payload.protocol === "NXDN") {
        const row: NxdnRow = { ...e.payload, ran: 0 };
  ...
  return (
    <AppShell
      ...
      footer={<RecordBar appId={"nxdn_rx" as any} format="jsonl" centerHz={parseFloat(freq) || undefined} />}
    >
      <DecoderTable
        headers={["RAN", "Source ID", "Dest ID", "Type"]}
        rows={rows}
        renderRow={(r) => [r.ran, r.source_id, r.talkgroup, <span style={{ color: "var(--text-secondary)" }}>{r.call_type}</span>]}
        emptyMessage="No calls yet — only unencrypted NXDN traffic is decoded."
      />
    </AppShell>
  );
}
