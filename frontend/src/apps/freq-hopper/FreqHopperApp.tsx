...
const WARNING_BANNER = (
  <div style={{ background: "#300", border: "1px solid #f44", borderRadius: 4, padding: "8px 12px", marginBottom: 12, color: "#f88", fontSize: 13 }}>
    <strong>INDOOR TEST ONLY</strong> — Frequency hopping transmissions require special authorization.
    All hop frequencies must be on your own licensed spectrum. Use only in a shielded environment.
  </div>
);
...
export function FreqHopperApp() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>Freq Hopper</h2>
      {WARNING_BANNER}
      <p style={{ color: "#aaa", fontSize: 13 }}>Frequency-hopping tone transmitter — one frequency per line (Hz)</p>

      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        <label>Frequencies (Hz)</label>
        <textarea
          value={freqList}
          onChange={(e) => setFreqList(e.target.value)}
          rows={6}
          style={inputStyle}
          placeholder="One frequency per line in Hz..."
        />
