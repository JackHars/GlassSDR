...
export function CwRxApp() {
  ...
  const handleTune = async (freqHz: number) => {
    ...
    if (running) {
      await stopApp();
      await startApp("cw_rx" as any, next);
    }
  };
    ...
    <AppShell
      ...
      controls={
        ...
        >
          ...
          <ControlField label={`Pitch ${tuning.bfo_hz} Hz`} size="md">
      ...
      footer={
        <>
          ...
          <RecordBar appId={"cw_rx" as any} format="wav" centerHz={tuning.center_hz} />
        </>
      }
    >
      <Waterfall
