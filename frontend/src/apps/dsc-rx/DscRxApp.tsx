...
export function DscRxApp() {
  ...
  const handleStart = async () => {
    await startApp("dsc_rx" as AppId, { center_hz: freq, lna_gain_db: 32, vga_gain_db: 20, amp_enabled: false });
    setRunning(true);
  };
  ...
  return (
    <AppShell
      ...
      footer={<RecordBar appId={"dsc_rx" as any} format="jsonl" centerHz={freq} />}
    >
      <DecoderTable
        headers={["MMSI", "Category"]}
        rows={messages}
        renderRow={(m) => [m.mmsi, m.category]}
        emptyMessage="No DSC traffic yet — Channel 70 (156.525 MHz) is the marine DSC frequency."
      />
    </AppShell>
  );
}
