...
const WARNING_BANNER = (
  <div style={{ background: "#220", border: "1px solid #fa0", borderRadius: 4, padding: "8px 12px", marginBottom: 12, color: "#fc8", fontSize: 13 }}>
    <strong>OWN DEVICES ONLY</strong> — LGE protocol transmissions must only target appliances you own and are licensed to operate.
  </div>
);
...
export function LgeTxApp() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>LGE TX</h2>
      {WARNING_BANNER}
      <p style={{ color: "#aaa", fontSize: 13 }}>LGE appliance protocol encoder — ISM band OOK/ASK</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        ...
        <label>Device Addr (hex)</label>
        <input type="text" value={deviceAddr} onChange={(e) => setDeviceAddr(e.target.value)} placeholder="e.g. 0x01" style={inputStyle} />

        <label>Command (hex)</label>
        <input type="text" value={command} onChange={(e) => setCommand(e.target.value)} placeholder="e.g. 0x00" style={inputStyle} />
