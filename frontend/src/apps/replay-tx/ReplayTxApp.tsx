...
const WARNING_BANNER = (
  <div style={{ background: "#220", border: "1px solid #fa0", borderRadius: 4, padding: "8px 12px", marginBottom: 12, color: "#fc8", fontSize: 13 }}>
    <strong>OWN DEVICES ONLY</strong> — Replaying captured signals at arbitrary frequencies may cause
    interference. Only use on frequencies and devices you own and are licensed to operate.
  </div>
);
...
export function ReplayTxApp() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2>Replay TX</h2>
      {WARNING_BANNER}
      <p style={{ color: "#aaa", fontSize: 13 }}>Replay captured IQ recordings via HackRF</p>

      <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: 8, maxWidth: 500 }}>
        <label>IQ File Path</label>
        <input type="text" value={filePath} onChange={(e) => setFilePath(e.target.value)} placeholder="/path/to/capture.iq" style={inputStyle} />
