...
export function RttyTxApp() {
  ...
  const handleTransmit = async () => {
    ...
    await startApp("rtty_tx" as any, {
      message, center_hz: parseFloat(frequency) || 0,
  ...
  return (
    <AppShell
      ...
      status={
        ...
        : <><span style={{color: "#999"}}>○</span> Idle · 45 baud · 170 Hz shift</>
      }
      controls={
        <ControlRow
          actions={
        ...
        >
          <ControlField label="Frequency (Hz)" size="lg">
            <input type="number" value={frequency} onChange={(e) => setFrequency(e.target.value)} placeholder="e.g. 14090000" />
          </ControlField>
      ...
      footer={<RecordBar appId={"rtty_tx" as any} format="iq" centerHz={parseFloat(frequency) || undefined} />}
    >
      ...
      <div className="app-shell__grow" style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
        <div style={{ flex: 1, padding: 16, background: "rgba(255,255,255,0.55)", border: "1px solid rgba(255,255,255,0.7)", borderRadius: 12, backdropFilter: "blur(16px)", display: "flex", flexDirection: "column", gap: 8, minHeight: 0 }}>
          ...
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Text to transmit…" style={{ flex: 1, resize: "none", minHeight: 120 }} />
        </div>
        {txStatus && (
